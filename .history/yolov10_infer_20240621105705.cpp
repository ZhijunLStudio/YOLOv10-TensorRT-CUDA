...
void save_input_buffer(float* input_buffer_device, int width, int height, cudaStream_t stream, std::string filename) {
    // 在CPU上分配内存
    float* input_buffer_host = new float[width * height * 3];

    // 将数据从GPU复制到CPU
    cudaMemcpyAsync(input_buffer_host, input_buffer_device, width * height * 3 * sizeof(float), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);

    // 将数据转换为cv::Mat格式
    cv::Mat img(height, width, CV_32FC3, input_buffer_host);

    // OpenCV保存图片
    cv::imwrite(filename, img * 255.0); // 乘以255.0是因为数据范围在0-1之间
...
int main(int argc, char** argv) {
    ...
    // unsigned char* input_buffer_device = nullptr;
    ...
    for (const auto& filename : filenames) {
        ...
        // 定义仿射变换矩阵
        AffineMat s2d, d2s;
        // 保存input_buffer_device到图片
        // save_input_buffer(input_buffer_device, INPUT_W, INPUT_H, stream, "preprocessed_image_before.png");

        // 预处理
        cuda_preprocess(img.data, img.cols, img.rows, input_buffer_device, INPUT_W, INPUT_H, stream, s2d, d2s);
        auto T3 = std::chrono::system_clock::now();
        // 保存input_buffer_device到图片
        // save_input_buffer(input_buffer_device, INPUT_W, INPUT_H, stream, "preprocessed_image.png");
        // 推理
