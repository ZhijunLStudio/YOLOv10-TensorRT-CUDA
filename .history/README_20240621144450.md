...
如果需要转int8，使用build_model/onnx_to_trt.py脚本进行导出即可，如下shi'li
```shell
python ./build_model/onnx_to_trt.py \
-m model_name.onnx -d int8 -v --img-size 640 --batch-size 1 \
...
python ./build_model/onnx_to_trt.py -m yolov10s.onnx -d int8 -v --img-size 640 --batch-size 1 --calib-img-dir /home/ubuntu/Downloads/coco_calib_data/
