...
void cuda_preprocess(uint8_t *src, int src_width, int src_height, float *dst, int dst_width, int dst_height,
                     cudaStream_t stream, AffineMat& s2d, AffineMatrix& d2s) {
    // 计算源图像的大小
    int img_size = src_width * src_height * 3;
    // 将源图像数据复制到主机上的 pinned 内存中
    memcpy(img_buffer_host, src, img_size);
    ...
    // 设置仿射变换矩阵
    m_dst2src.v0 = d2s.value[0];
    m_dst2src.v1 = d2s.value[1];
    m_dst2src.v2 = d2s.value[2];
    m_dst2src.v3 = d2s.value[3];
    m_dst2src.v4 = d2s.value[4];
    m_dst2src.v5 = d2s.value[5];
    ...
    // normDevice(1, m_input_rgb_device, dst_width, dst_height, m_input_norm_device, dst_width, dst_height, m_param);
