...
void prepare_buffer(ICudaEngine* engine, float** input_buffer_device, float** output_buffer_device,
    ...
    std::cout << "out_dims.d[0]: " << out_dims.d[0] << std::endl;
...
void infer(IExecutionContext& context, cudaStream_t& stream, void** buffers, float* output, int batchsize,
           float* decode_ptr_host, float* decode_ptr_device, int model_bboxes, size_t& kOutputSize) {
...
int main(int argc, char** argv) {
    ...
    size_t kOutputSize = 1;

    prepare_buffer(engine, &input_buffer_device, &output_buffer_device, &output_buffer_host, &decode_ptr_host, &decode_ptr_device, kOutputSize);
    ...
    for (const auto& filename : filenames) {
        ...
        // 定义仿射变换矩阵并显式分配内存
        // cv::Mat m2x3_s2d = cv::Mat::zeros(2, 3, CV_32F);
        // cv::Mat m2x3_d2s = cv::Mat::zeros(2, 3, CV_32F);
        AffineMatrix s2d, d2s;
        // std::cout << "before m2x3_d2s:\n" << m2x3_d2s << std::endl; 


        // 使用 cuda_preprocess 进行预处理
        cuda_preprocess(img.data, img.cols, img.rows, input_buffer_device, kInputW, kInputH, stream, s2d, d2s);
        auto T3 = std::chrono::system_clock::now();
        
        infer(*context, stream, (void**)buffers, output_buffer_host, kBatchSize, decode_ptr_host, decode_ptr_device, kMaxNumOutputBbox, kOutputSize);
        auto T4 = std::chrono::system_clock::now();

        // 后处理
        int batch_size = 1; // 假设 batch size 为 1
        int num_boxes = 300; // 每张图像的预测框数量
        int num_coords = 6; // 每个预测框的维度数量
        auto results = postProcess(output_buffer_host, batch_size, num_boxes, num_coords);
        ...
        for (int b = 0; b < batch_size; ++b) {
            ...
            for (const auto& det : results[b]) {
                std::cout << "Left: " << det.left << ", Top: " << det.top
                        << ", Right: " << det.right << ", Bottom: " << det.bottom
                        << ", Confidence: " << det.confidence << ", Class ID: " << det.class_id << "\n";
            }
        }
        // std::cout << "after m2x3_d2s:\n" << m2x3_d2s << std::endl;    
